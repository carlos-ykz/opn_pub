...
# utilize o poder objetivo?") para mostrar o caminho completo de uma
...
# objetivo) em uma variável chamada "objetivo", em três sub-passos:
#
...
df <- df %>%
  ...
  mutate(
    ...
    objetivo_n = as.factor(
      as.numeric(gsub("[^0-9]", "", as.character(objetivo))) - 1
    )
  )
...
ggsave(
  filename = "Opinião sobre a confiança nos EUA.png", 
  width = 8,                          
  height = 4,                          
  units = "in",                        
  dpi = 300)
